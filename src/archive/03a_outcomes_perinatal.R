...
if (as.integer(cfg$parent_income_year_max) < 2011) {
  income_household <-
    income_household %>% 
    mutate(household_income = ifelse(household_income == 999999999, NA,
                                     household_income)) 
  
} else {
  income_household <-
    income_household %>% 
    mutate(household_income = ifelse(household_income == 9999999999, NA, 
                                     household_income)) 
}
...
# remove parents income and replace with househould income
cohort_dat <- 
  cohort_dat %>%
  select(
  -c(income_n_ma, income_n_pa, income_ma, income_pa, income_parents_1log, 
     income_parents, income_parents_rank, income_parents_perc)
) %>%
