...
child_adres_date <- 
  cohort_dat %>%
  mutate(adres_date = ifelse(month(birthdate) %in% c(1:9), 
                             (as.numeric(format(birthdate, "%Y")) + cfg$child_live_jan_sep),
                             (as.numeric(format(birthdate, "%Y")) + cfg$child_live_oct_dec))
         ) %>%
